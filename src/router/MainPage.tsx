...
import { fetchingData, fetchingMoreData } from "../features/data-slice";
import { useDispatch, useSelector } from "react-redux";
import BookList from "../components/BookList";
import styles from "./MainPage.module.css";
import { dataActions } from "../features/data-action";
import { RootState } from "../app/store";
import { MoreBooksURLData } from "../models/IDataModels";
...
const MainPage: React.FC = () => {
  ...
  const moreBooksURLData = useSelector(
    (state: RootState) => state.fetchData.moreBooksURLData
  );
  ...
  const fetchMoreHandler = () => {
    dispatch(
      fetchingMoreData(numberToAdd, moreSearchInput, orderBy, categoryBy)
    );
    setNumberToAdd((prev) => prev + 30);
  };
  const orderByHandler = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const categoryByHandler = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const dataForBackup: MoreBooksURLData = [
    numberToAdd,
    moreSearchInput,
    orderBy,
    categoryBy,
    ...
    <div className={styles.mainOuter}>
      <div className={styles.header}>
        ...
        <div className={styles.selection}>
          <div className={styles.byOrder}>
            Sorted by
            <select onChange={orderByHandler} id="order" name="order">
          ...
          <div className={styles.byCategory}>
            Categories
            <select onChange={categoryByHandler} id="category" name="category">
